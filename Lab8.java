...
class MovieDatabase {
    ...
    public boolean hasActorWithoutMovies() {
        for (Actor actor : actors) { // Using a for-each loop (c)
    ...
    public Set<Actor> getCoActors(Actor targetActor) {
        ...
        Iterator<Movie> movieIterator = targetActor.getMovies().iterator(); // Using a typed iterator (b)
    ...
    public Movie getMovieWithMostActors() {
        ...
        Iterator<Movie> iterator = movies.iterator(); // Using an untyped iterator (a)
